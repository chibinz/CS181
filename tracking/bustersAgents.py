...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        ghostPos = [max(posDist, key=posDist.get)
                    for posDist in livingGhostPositionDistributions]

        closest = sorted(ghostPos, key=lambda pos: self.distancer.getDistance(pos, pacmanPosition))[0]
        act = sorted(legal, key=lambda a: self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, a), closest))[0]
